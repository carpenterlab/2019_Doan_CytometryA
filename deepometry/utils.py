...
def load(directories, patients_to_test, convert=True, sample=None):
    """
    ...
    print('Before exclusion: ',len(paths))

    paths = [x for x in paths if numpy.all([not z in x for z in patients_to_test])]
    print('After exclusion: ',len(paths))    
